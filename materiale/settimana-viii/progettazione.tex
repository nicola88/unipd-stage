...
\begin{document}
	...
	La classe \textit{\nameref{sec:stage:design:model:content}} rappresenta un generico contenuto informativo pubblicato dagli utenti nella piattaforma.

	\subsection[Metadata]{Metadata | Metadati dei contenuti}
	\label{sec:stage:design:model:metadata}
	L'interfaccia \textit{\nameref{sec:stage:design:model:metadata}} rappresenta una meta-informazione generica associata ad un contenuto informativo.

	\subsection[Emotion]{Emotion | Emozioni}
	\label{sec:stage:design:model:metadata-emotion}
	La classe \textit{\nameref{sec:stage:design:model:metadata-emotion}} rappresenta un'emozione associabile ad un contenuto ed implementa l'interfaccia \textit{\nameref{sec:stage:design:model:metadata}}.

	\subsection[Intention]{Intention | Intenzione}
	\label{sec:stage:design:model:metadata-intention}
	La classe \textit{\nameref{sec:stage:design:model:metadata-intention}} rappresenta un'intenzione associabile ad un contenuto ed implementa l'interfaccia \textit{\nameref{sec:stage:design:model:metadata}}.

	\subsection[Interest]{Interest | Interesse}
	\label{sec:stage:design:model:metadata-interest}
	La classe \textit{\nameref{sec:stage:design:model:metadata-interest}} rappresenta gli interessi cui è associabile un contenuto ed implementa l'interfaccia \textit{\nameref{sec:stage:design:model:metadata}}.

	\subsection[Topic]{Topic | Argomento}
	\label{sec:stage:design:model:metadata-topic}
	La classe \textit{\nameref{sec:stage:design:model:metadata-topic}} rappresenta un argomento cui può appartenere un contenuto ed implementa l'interfaccia \textit{\nameref{sec:stage:design:model:metadata}}.
	...
	La classe \textit{\nameref{sec:stage:design:model:metadata-user}} rappresenta l'utente che ha pubblicato il contenuto ed implementa l'interfaccia \textit{\nameref{sec:stage:design:model:metadata}}.

	\subsection[Entity]{Entity | Entità del dominio}
	\label{sec:stage:design:model:entity}
	La classe \textit{\nameref{sec:stage:design:model:entity}} rappresenta un'entità del dominio della piattaforma, cui è associata un'etichetta - in una specifica accezione - che la identifica univocamente nell'ambito della piattaforma (\textit{\nameref{sec:stage:design:model:label}}).

	\subsection[EntityType]{EntityType | Tipo di entità}
	\label{sec:stage:design:model:entity-type}
	La classe \textit{\nameref{sec:stage:design:model:entity-type}} rappresenta i tipi di entità del dominio.

	\subsection[Label]{Label | Etichetta del dizionario}
	\label{sec:stage:design:model:label}
	La classe \textit{\nameref{sec:stage:design:model:label}} modella le etichette del dizionario della piattaforma, ciascuna delle quali può possedere molteplici accezioni (\textit{\nameref{sec:stage:design:model:meaning}}).

	\subsection[Meaning]{Meaning | Accezione di un'etichetta}
	\label{sec:stage:design:model:meaning}
	La classe \textit{\nameref{sec:stage:design:model:meaning}} rappresenta un'accezione di un'etichetta (\textit{\nameref{sec:stage:design:model:label}}) e si riferisce un'entità del dominio (\textit{\nameref{sec:stage:design:model:entity}}).
	...
	La classe \textit{\nameref{sec:stage:design:model.criteria:list-criterion}} rappresenta un criterio di classificazione basato su una lista di valori, ciascuno dei quali può essere ammissibile o bloccato. 

	Ciascuna istanza viene utilizzata in combinazione con un filtro \textit{\nameref{sec:stage:design:model.filter:list-filter}} o \textit{\nameref{sec:stage:design:model.filter:switch-filter}}, fornendo i valori e i parametri di configurazione iniziale.

	\subsection[CRange]{CRange | Criterio basati su un intervallo di valori}
	\label{sec:stage:design:model.criteria:range-criterion}
	La classe \textit{\nameref{sec:stage:design:model.criteria:range-criterion}} rappresenta un criterio di classificazione basato su un intervallo di valori.

	Ciascuna istanza viene utilizzata in combinazione con un filtro \textit{\nameref{sec:stage:design:model.filter:range-filter}} o \textit{\nameref{sec:stage:design:model.filter:switch-filter}}, fornendo i valori e i parametri di configurazione iniziale.

	\subsection[CValue]{CValue | Filtro basato su una soglia di valore}
	\label{sec:stage:design:model.criteria:value-criterion}
	La classe \textit{\nameref{sec:stage:design:model.criteria:value-criterion}} rappresenta un criterio di classificazione basato su una soglia di valore.

	Ciascuna istanza viene utilizzata in combinazione con un filtro \textit{\nameref{sec:stage:design:model.filter:value-filter}} o \textit{\nameref{sec:stage:design:model.filter:switch-filter}}, fornendo i valori e i parametri di configurazione iniziale.
	...
	Ciascun filtro è associato ad un possibile criterio di classificazione (\textit{\nameref{sec:stage:design:model.criteria:criteria}}), che partiziona automaticamente l'insieme dei possibili valori in due sottoinsiemi: ammessi o bloccati.
	...
	La classe \textit{\nameref{sec:stage:design:model.filter:filter-factory}} offre un'interfaccia per creare filtri (\textit{\nameref{sec:stage:design:model.filter:filter}}) basati sui possibili criteri (\textit{\nameref{sec:stage:design:model.criteria:criteria}}) in modo trasparente rispetto al client.

	\begin{table}[ht]
	\centering
	\begin{tabular}{|l|l|l|l|}
	\hline
	& \textsc{\nameref{sec:stage:design:model.criteria:criteria}} & \textsc{\nameref{sec:stage:design:model.filter:filter}} & \textsc{\nameref{sec:stage:design:view.filter:filter}} \\ \hline
	\textit{Argomento}  & \nameref{sec:stage:design:model.criteria:list-criterion} & \nameref{sec:stage:design:model.filter:list-filter} & \nameref{sec:stage:design:model.filter:list-filter} \\ \hline
	\textit{Attinenza} & \nameref{sec:stage:design:model.criteria:value-criterion} & \nameref{sec:stage:design:model.filter:value-filter} & \nameref{sec:stage:design:model.filter:value-filter} \\ \hline
	\textit{Autore} & \nameref{sec:stage:design:model.criteria:list-criterion} & \nameref{sec:stage:design:model.filter:list-filter} & \nameref{sec:stage:design:model.filter:list-filter} \\ \hline
	\textit{Data di pub.} & \nameref{sec:stage:design:model.criteria:range-criterion} & \nameref{sec:stage:design:model.filter:range-filter} & \nameref{sec:stage:design:model.filter:range-filter} \\ \hline
	\textit{Emozioni} & \nameref{sec:stage:design:model.criteria:list-criterion} & \nameref{sec:stage:design:model.filter:list-filter} & \nameref{sec:stage:design:model.filter:list-filter} \\ \hline
	\textit{Giudizi} & \nameref{sec:stage:design:model.criteria:value-criterion} & \nameref{sec:stage:design:model.filter:value-filter} & \nameref{sec:stage:design:model.filter:value-filter} \\ \hline
	\textit{Intenzioni} & \nameref{sec:stage:design:model.criteria:list-criterion} & \nameref{sec:stage:design:model.filter:list-filter} & \nameref{sec:stage:design:model.filter:list-filter} \\ \hline
	\textit{Interessi} & \nameref{sec:stage:design:model.criteria:list-criterion} & \nameref{sec:stage:design:model.filter:switch-filter} & \nameref{sec:stage:design:model.filter:switch-filter} \\ \hline
	\textit{Tipo} & \nameref{sec:stage:design:model.criteria:list-criterion} & \nameref{sec:stage:design:model.filter:list-filter}  & \nameref{sec:stage:design:model.filter:list-filter} \\ \hline
	\end{tabular}
	\caption{Dipendenze tra classi rilevanti per \textit{\nameref{sec:stage:design:model.filter:filter-factory}}}
	\label{tab:stage:design:filter-factory}
	\end{table}
	...
	La classe \textit{\nameref{sec:stage:design:model.filter:switch-filter}} modella un filtro il cui partizionamento è predefinito e invariabile e che l'utente può solamente abilitare o disattivare. Ciascuna istanza è associata ad un qualsiasi criterio (\textit{\nameref{sec:stage:design:model.criteria:criteria}}).
	...
	La classe \textit{\nameref{sec:stage:design:model.timeline:timeline}} rappresenta la linea del tempo, suddivisa in intervalli (\textit{\nameref{sec:stage:design:model.timeline:time-unit}}) di ampiezza fissa - personalizzabile dall'utente in termini di unità (giorni, settimane, mesi, anni) e scala (1, 2, \ldots) - in cui vengono collocati i risultati della ricerca o i contenuti di una discussione.

	\subsection[TimeUnit]{TimeUnit | Unità temporale}
	\label{sec:stage:design:model.timeline:time-unit}
	La classe \textit{\nameref{sec:stage:design:model.timeline:time-unit}} rappresenta un generico intervallo di tempo, che consente di reperire i contenuti corrispondenti ai risultati di ricerca relativi ad una discussione e pubblicati nel periodot corrispondente.
	...
	\begin{description}
	\item[Autore] L'autore del contenuto è l'utente (\textit{\nameref{sec:stage:design:model:metadata-user}}) che lo ha pubblicato all'interno della piattaforma e viene rappresentato testualmente mediante il suo \textit{nome utente} o \textit{proprio}.
	...
	\item[Tipo] Il tipo di contenuto (\textit{\nameref{sec:stage:design:model:metadata-type}}) viene rappresentato graficamente variando la forma\footnote{Le forme utilizzate sono elementari per garantire l'immediata riconoscibilità da parte di qualsiasi tipo di utente, evitando l'impiego di forme potenzialmente ambigue o ignote a seconda del suo profilo sociale, culturale, geografico, \ldots} dell'elemento grafico.
	...
	\begin{description}
	\item[Argomento] Ciascun contenuto può riferirsi al più ad un argomento (\textit{\nameref{sec:stage:design:model:metadata-topic}}), rappresentabile in maniera grafica (colore o simbolo) o testuale.
	\item[Emozioni] A ciascun contenuto possono essere associate diverse emozioni (\textit{\nameref{sec:stage:design:model:metadata-emotion}}), che esprimono lo stato d'animo dell'autore al momento della sua redazione o pubblicazione. La lista delle emozioni associate ad un contenuto viene visualizzata in formato testuale.
	...
	\item[Intenzioni] A ciascun contenuto possono essere associate delle intenzioni (\textit{\nameref{sec:stage:design:model:metadata-intention}}), che possano fornire delle linee guida interpretative ai lettori. La lista delle intenzioni associate ad un contenuto viene visualizzata in formato testuale.
	\end{description}

	\subsection[CVAnswer]{CVAnswer | Risposta}
	\label{sec:stage:design:view:answer}
	La classe \textit{\nameref{sec:stage:design:view:answer}} rappresenta il componente grafico che visualizza un contenuto di tipo \textit{risposta} ed estende la classe \textit{\nameref{sec:stage:design:view:content-widget}}.

	\subsection[CVEvent]{CVEvent | Evento}
	\label{sec:stage:design:view:event}
	La classe \textit{\nameref{sec:stage:design:view:event}} rappresenta il componente grafico che visualizza un contenuto di tipo \textit{evento} ed estende la classe \textit{\nameref{sec:stage:design:view:content-widget}}.

	\subsection[CVMessage]{CVMessage | Comunicazione privata}
	\label{sec:stage:design:view:message}
	La classe \textit{\nameref{sec:stage:design:view:message}} rappresenta il componente grafico che visualizza un contenuto di tipo \textit{comunicazione privata} ed estende la classe \textit{\nameref{sec:stage:design:view:content-widget}}.

	\subsection[CVQuestion]{CVQuestion | Domanda}
	\label{sec:stage:design:view:question}
	La classe \textit{\nameref{sec:stage:design:view:question}} rappresenta il componente grafico che visualizza un contenuto di tipo \textit{domanda} ed estende la classe \textit{\nameref{sec:stage:design:view:content-widget}}.

	\subsection[CVReview]{CVReview | Recensione}
	\label{sec:stage:design:view:review}
	La classe \textit{\nameref{sec:stage:design:view:review}} rappresenta il componente grafico che visualizza un contenuto di tipo \textit{recensione} ed estende la classe \textit{\nameref{sec:stage:design:view:content-widget}}.

	\subsection[CVTalk]{CVTalk | Discorso}
	\label{sec:stage:design:view:talk}
	La classe \textit{\nameref{sec:stage:design:view:talk}} rappresenta il componente grafico che visualizza un contenuto di tipo \textit{discorso} ed estende la classe \textit{\nameref{sec:stage:design:view:content-widget}}.

	\subsection[CVThought]{CVThought | Pensiero}
	\label{sec:stage:design:view:thought}
	La classe \textit{\nameref{sec:stage:design:view:thought}} rappresenta il componente grafico che visualizza un contenuto di tipo \textit{pensiero} ed estende la classe \textit{\nameref{sec:stage:design:view:content-widget}}.
...
%	\end{itemize}
	...
	La classe \textit{\nameref{sec:stage:design:controller:content}} gestisce le operazioni relative al reperimento e alla consultazione dei contenuti informativi, sia come risultati di una ricerca sia come elementi di una discussione.

	\subsection[EntityController]{EntityController | Gestione delle entità}
	\label{sec:stage:design:controller:entity}
	La classe \textit{\nameref{sec:stage:design:controller:entity}} gestisce le operazioni relative alle entità del dominio individuate al termine della ricerca e con cui l'utente può interagire.

	\subsection[TimelineController]{TimelineController | Gestione della cronologia}
	\label{sec:stage:design:controller:timeline}
	La classe \textit{\nameref{sec:stage:design:controller:timeline}} gestisce le operazioni riguardanti la navigazione temporale dei contenuti.
	...
	\label{sec:stage:design:controller:filter}
	La classe \textit{\nameref{sec:stage:design:controller:filter}} gestisce la creazione, il caricamento e l'interazione dell'utente con i filtri.

	\subsection[FCList]{FCList | Gestione dei filtri a lista di valori}
	\label{sec:stage:design:controller:filter-list}
	La classe \textit{\nameref{sec:stage:design:controller:filter-list}} estende la classe \textit{\nameref{sec:stage:design:controller:filter}} e gestisce i filtri di tipo \textit{\nameref{sec:stage:design:model.filter:list-filter}}.

	\subsection[FCRange]{FCRange | Gestione dei filtri ad intervallo di valori}
	\label{sec:stage:design:controller:filter-range}
	La classe \textit{\nameref{sec:stage:design:controller:filter-range}} estende la classe \textit{\nameref{sec:stage:design:controller:filter}} e gestisce i filtri di tipo \textit{\nameref{sec:stage:design:model.filter:range-filter}}.

	\subsection[FCSwitch]{FCSwitch | Gestione dei filtri ad interruttore}
	\label{sec:stage:design:controller:filter-switch}
	La classe \textit{\nameref{sec:stage:design:controller:filter-switch}} estende la classe \textit{\nameref{sec:stage:design:controller:filter}} e gestisce i filtri di tipo \textit{\nameref{sec:stage:design:model.filter:switch-filter}}.

	\subsection[FCValue]{FCValue | Gestione dei filtri a soglia di valore}
	\label{sec:stage:design:controller:filter-value}
	La classe \textit{\nameref{sec:stage:design:controller:filter-value}} estende la classe \textit{\nameref{sec:stage:design:controller:filter}} e gestisce i filtri di tipo \textit{\nameref{sec:stage:design:model.filter:value-filter}}.
	...
	\label{sec:stage:design:controller:search}
	La classe \textit{\nameref{sec:stage:design:controller:search}} gestisce le operazioni connesse alla ricerca di contenuti informativi nella piattaforma.
	...
	La classe \textit{\nameref{sec:stage:design:controller.search:result-update}} rappresenta l'interfaccia delle operazioni inerenti l'aggiornamento dei risultati di ricerca:
	\begin{itemize}
	...
	La classe \textit{\nameref{sec:stage:design:controller.search:result-update-list-refine}} incapsula il comando di aggiornamento dei risultati di ricerca visualizzati a fronte dell'alterazione di un filtro \textit{\nameref{sec:stage:design:model.filter:list-filter}}.
	...
	La classe \textit{\nameref{sec:stage:design:controller.search:result-update-range-refine}} incapsula il comando di aggiornamento dei risultati di ricerca visualizzati a fronte dell'alterazione di un filtro \textit{\nameref{sec:stage:design:model.filter:range-filter}}.
	...
	La classe \textit{\nameref{sec:stage:design:controller.search:result-update-switch-refine}} incapsula il comando di aggiornamento dei risultati di ricerca visualizzati a fronte dell'abilitazione o disattivazione di un filtro \textit{\nameref{sec:stage:design:model.filter:switch-filter}}.
	...
	La classe \textit{\nameref{sec:stage:design:controller.search:result-update-value-refine}} incapsula il comando di aggiornamento dei risultati di ricerca visualizzati a fronte dell'alterazione di un filtro \textit{\nameref{sec:stage:design:model.filter:value-filter}}.
	...
	La classe \textit{\nameref{sec:stage:design:controller.search:result-update-load-discussion}} incapsula il comando di aggiornamento dei risultati di ricerca a fronte della visualizzazione della discussione generata da un contenuto informativo visualizzato.
	...
	La classe \textit{\nameref{sec:stage:design:controller.search:result-update-load-entity}} incapsula il comando di aggiornamento dei risultati di ricerca a fronte dell'eliminazione di un'entità.
	...
	\begin{itemize}
	\item \nameref{sec:stage:design:controller.search:result-update}
	\begin{itemize}
	\item \nameref{sec:stage:design:controller.search:result-update-list-refine}
	\item \nameref{sec:stage:design:controller.search:result-update-range-refine}
	\item \nameref{sec:stage:design:controller.search:result-update-switch-refine}
	\item \nameref{sec:stage:design:controller.search:result-update-value-refine}
	\item \nameref{sec:stage:design:controller.search:result-update-load-discussion}
	\item \nameref{sec:stage:design:controller.search:result-update-load-entity}
	\end{itemize}
	\end{itemize}
	...
	\begin{itemize}
	\item \nameref{sec:stage:design:model.criteria:criteria-model}
	\item \nameref{sec:stage:design:model.filter:filter}
	\item \nameref{sec:stage:design:model.search:provider-model}
	\item \nameref{sec:stage:design:model.search:search}
	\item \nameref{sec:stage:design:view.filter:filter-container}
	\item \nameref{sec:stage:design:view.search:search-container}
	\item \nameref{sec:stage:design:view.timeline:timeline-view}
	\end{itemize}

	\section{Singleton}
	\label{sec:stage:design:pattern:singleton}
	\begin{itemize}
	\item \nameref{sec:stage:design:controller:main-controller}
	\end{itemize}
	...
	Le informazioni relative al tracciamento - requisiti verranno rese disponibili successivamente in un file \textit{tracciamento.ods}.
